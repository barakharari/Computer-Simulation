...
public class FullAssembler extends Assembler {
	...
	public int assemble(String inputFileName, String outputFileName, TreeMap<Integer, String> errors) {
		...
		//every pasm file starts with code, so we initially have inCode equal to true
		...
		try (Scanner input = new Scanner(new File(inputFileName))) {
			...
			while(input.hasNextLine()) {
				...
				//check for errors associated with non blank lines in this if block
					...
					if (line.charAt(0) == ' ' || line.charAt(0) == '\t') {
						errors.put(lineNum, "Error on line " + lineNum + ": Line starts with illegal white space");
						retVal = lineNum;
					}
					...
					if(blankLineFound) {
						errors.put(firstBlankLineNum, "Error on line " + firstBlankLineNum + ": Illegal blank line in the source file");
					...
					if(line.trim().toUpperCase().contains("DATA")){
						...
						if (inCode){
							...
							errors.put(lineNum, "Error on line " + lineNum + ": duplicate Data delimeter");
							retVal = lineNum;
						}
						...
						if(!line.trim().contains("DATA")) {
							errors.put(lineNum, "Error on line " + lineNum + ": DATA delimeter must be all uppercase");
							retVal = lineNum;
						}
					}
				} 
				...
				if(line.length() > 0){
					...
					if(Model.OPCODES.containsKey(parts[0].toUpperCase())) {
						...
						//exactly 1 argument
							...
							if(parts.length > 2){
								String retString = "Error on line " + lineNum + ": this mnemonic has too many arguments: ";
								for (int i = 1; i < parts.length; i++) {
									retString += parts[i];
									retString += " ";
								}
								errors.put(lineNum, retString);
								retVal = lineNum;
							}
			...
			for(String line : data){
				...
				if(line.length() > 0 && !line.toUpperCase().contains("DATA")){
					...
					if(parts.length != 2){
						errors.put(lineNum, "Error on line " + lineNum + ": data format does not consist of two numbers");
						retVal = lineNum;
					}
